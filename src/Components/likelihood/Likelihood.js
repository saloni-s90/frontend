...
// Likelihood is displayed using bar graph
const Likelihood = ({ serverData }) => {

  let uniqueSectors = [];

  //using forEach because it doesn't return any array and we are adding unique sector names to an array
  serverData.forEach((i) => {
    if (!uniqueSectors.includes(i.likelihood) && i.likelihood !== "") {
      uniqueSectors.push(i.likelihood);
  ...
  const sectorCount = uniqueSectors.map((item) => {
    return {
      sector: item,
      count: serverData.filter((i) => i.likelihood === item).length
